...
void writeCoords() {
  ...
  ArrayList<String> coords = pvectToString(wormVertices);
  ...
  /* String[] coordArray = wormVertices.toArray(); */
  println(coordArray);
  saveStrings (fileName, coordArray);
}


ArrayList<String> pvectToString(ArrayList<PVector> pvects){
  ArrayList<String> strings = new ArrayList<String>();
  for(PVector pv : pvects){
    //coords.add(str(random(0, width)) + ", " + str(random(0, width)) + ", " + str(random(0, width)));
    strings.add(pv.x + ", " + pv.y + ", " + pv.z);
  }
